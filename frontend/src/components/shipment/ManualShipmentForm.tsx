...
export default function ManualShipmentForm({ orderId }: ManualShipmentFormProps) {
  ...
  useEffect(() => {
    const fetchOrder = async () => {
      ...
      if (res.data.code !== 0) {
        ...
        navigate(`/orders/${orderId }`); // ⭐ 回到订单详情页
        return;
      }
  ...
  const handleSubmit = async () => {
    ...
    request.post(`/orders/${orderId}/shipments`, payload).then((res) => {
      if (res.data.code === 0) {
