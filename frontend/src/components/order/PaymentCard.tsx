...
export default function PaymentCard({ order, onRefresh }: Props) {
  const { t } = useTranslation();

  const [open, setOpen] = useState(false);

  const handleCreatePayment = async (data: any) => {
    await createPayment(order.id, data);
    ...
    <Card sx={{ borderRadius: 3 }}>
      <CardContent>
        ...
        <Stack spacing={1} sx={{ mb: 3 }}>
          <Typography>
            {t("payment.card.orderAmount")}： ￥{order.totalAmount}
          </Typography>
        ...
        <PaymentRecordTable records={order.paymentRecords || []} />
      </CardContent>
