...
export default function CreateOrderPage() {
  ...
  const handleItemChange = async (index: number, key: string, value: any) => {
    ...
    if (key === "product_id") {
      ...
      const sku = skuOptions[currentItem.product_id]?.find(
        (s) => s.id === value,
      );
      newItems[index] = {
        ...currentItem,
  ...
  return (
    <Box>
      <Typography sx={{ variant: "h5", mb: 2 }}>{t("order.create")}</Typography>
            ...
            >
              {selectedCustomer?.addresses?.map((a: any) => (
                <MenuItem key={a.id} value={a.id}>
                  {a.name} / {a.phone} - {a.province} {a.city} {a.district}{" "}
                  {a.address}
                </MenuItem>
              ))}
            </TextField>
            ...
            {items.map((item, index) => (
              ...
              >
                ...
                <TextField
                  ...
                  value={item.sku_id}
                  onChange={(e) =>
                    handleItemChange(index, "sku_id", Number(e.target.value))
                  }
